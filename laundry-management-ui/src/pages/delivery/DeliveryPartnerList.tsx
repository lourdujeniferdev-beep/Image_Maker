...
export default function DeliveryPartnerList() {
  ...
  const fetchData = () => {
    ...
    Promise.all([mockApi.partners.list(), mockApi.orders.list()]).then(([partnerList, orderList]) => {
      setPartners(partnerList.filter((p) => p.type === "delivery"));
      setOrders(orderList);
      setLoading(false);
    });
  };
  ...
  const handleOpenAssign = (partner: Partner) => {
    ...
    const readyOrders = orders.filter((o) => o.status === "Ready For Delivery" && !o.deliveryPartnerId);
    setSelectedOrderId(readyOrders[0]?.id || 0);
  ...
  return (
    <div className="space-y-6 text-left">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-3xl font-bold text-slate-800">Delivery Logistics Partners</h1>
          <p className="text-slate-500 text-sm">
            Manage dispatching agents responsible for returning clean laundry to customer doorsteps.
          </p>
        </div>
      </div>
      ...
      <Table
        ...
        searchPlaceholder="Search delivery agents by name, phone..."
      ...
      {/* Assign Order Modal */}
      <Modal isOpen={activeModal === "assign"} onClose={() => setActiveModal(null)} title="Dispatch Delivery Courier">
        <form onSubmit={handleAssignSubmit} className="space-y-4">
          {selectedPartner && (
            <div>
              <p className="text-sm text-slate-500 mb-4">
                Assigning a laundry package delivery task to <span className="font-bold text-slate-700">{selectedPartner.name}</span>.
              </p>

              {readyOrders.length > 0 ? (
                <div>
                  <label className="block text-sm font-semibold text-slate-700 mb-1">Select Ready Order *</label>
                  <select
                ...
                <div className="bg-amber-50 text-amber-700 p-3 rounded-lg border text-xs font-semibold text-center">
                  There are no unassigned "Ready For Delivery" orders in queue.
                </div>
              )}
            </div>
          )}
      ...
      {/* View Details Modal */}
      <Modal isOpen={activeModal === "view"} onClose={() => setActiveModal(null)} title="Delivery Partner Details">
        {selectedPartner && (
          <div className="space-y-6">
            <div className="flex items-center gap-4 bg-slate-50 dark:bg-slate-900/40 p-4 rounded-xl">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400">Delivery Logistics Courier</p>
              </div>
            </div>
